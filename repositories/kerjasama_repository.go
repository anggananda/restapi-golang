...
type KerjasamaMongoRepository struct {
	DB *mongo.Database
}

func NewKerjasamaMongoRepository(db *mongo.Database) interfaces.KerjasamaRepository {
	return &KerjasamaMongoRepository{
		DB: db,
	}
}

func (repo *KerjasamaMongoRepository) getCollectionByYear(year string) *mongo.Collection {
	return repo.DB.Collection(fmt.Sprintf("kerjasama_%s", year))
}

func (repo *KerjasamaMongoRepository) GetKerjasamaFiltered(ctx context.Context, kodeFakultas, kodeJurusan, kodeProdi, tahun, search string, page, limit int) ([]models.Kerjasama, int64, error) {
	skip := (page - 1) * limit
	filter := bson.M{}
	if kodeFakultas != "" {
