...
type HkiMongoRepository struct {
	DB *mongo.Database
}

func NewHkiMongoRepository(db *mongo.Database) interfaces.HkiRepository {
	return &HkiMongoRepository{
		DB: db,
	}
}

func (repo *HkiMongoRepository) getCollectionByYear(year string) *mongo.Collection {
	return repo.DB.Collection(fmt.Sprintf("hki_%s", year))
}

func (repo *HkiMongoRepository) GetHkiFiltered(ctx context.Context, kodeFakultas, kodeJurusan, kodeProdi, tahun, semester, search string, page, limit int) ([]models.Hki, int64, error) {
	skip := (page - 1) * limit
	filter := bson.M{}
