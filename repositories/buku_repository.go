...
type BukuMongoRepository struct {
	DB *mongo.Database
}

func NewBukuMongoRepository(db *mongo.Database) interfaces.BukuRepository {
	return &BukuMongoRepository{
		DB: db,
	}
}

func (repo *BukuMongoRepository) getCollectionByYear(year string) *mongo.Collection {
	return repo.DB.Collection(fmt.Sprintf("buku_%s", year))
}

func (repo *BukuMongoRepository) GetBukuFiltered(ctx context.Context, kodeFakultas, kodeJurusan, kodeProdi, tahun, semester, search string, page, limit int) ([]models.Buku, int64, error) {
	skip := (page - 1) * limit
	filter := bson.M{}
