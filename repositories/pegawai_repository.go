...
type PegawaiMongoRepository struct {
	Collection *mongo.Collection
}

func NewPegawaiMongoRepository(db *mongo.Database) interfaces.PegawaiRepository {
	return &PegawaiMongoRepository{
		Collection: db.Collection("pegawai"),
	}
}

func (repo *PegawaiMongoRepository) GetDetailPegawai(ctx context.Context, niu string) (*models.PegawaiHistory, error) {
	var pegawai models.PegawaiHistory

	if err := repo.Collection.FindOne(ctx, bson.M{"no_induk_undiksha": niu}).Decode(&pegawai); err != nil {
		return nil, err
	}
...
func (repo *PegawaiMongoRepository) GetPegawaiHistoryFiltered(ctx context.Context, kodeFakultas, kodeJurusan, kodeProdi, search string, tahun, statusPegawai, statusKeaktifan, page, limit int) ([]models.PegawaiHistoryResponse, int64, error) {
	normalMatch := bson.M{
		"history.tahun": tahun,
	}
	...
	var results []models.PegawaiHistoryResponse
	var total int64
