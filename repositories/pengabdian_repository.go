...
type PengabdianMongoRepository struct {
	DB *mongo.Database
}

func NewPengabdianMongoRepository(db *mongo.Database) interfaces.PengabdianRepository {
	return &PengabdianMongoRepository{
		DB: db,
	}
}

func (repo *PengabdianMongoRepository) getCollectionByYear(year string) *mongo.Collection {
	return repo.DB.Collection(fmt.Sprintf("pengabdian_%s", year))
}

func (repo *PengabdianMongoRepository) GetPengabdianFiltered(ctx context.Context, kodeFakultas, kodeJurusan, kodeProdi, tahun, semester, search string, page, limit int) ([]models.Pengabdian, int64, error) {
	skip := (page - 1) * limit
	filter := bson.M{}
